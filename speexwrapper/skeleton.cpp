...
int add_message_header_field(fisbone_packet *fp, 
    ...
    if (fp->message_header_fields == NULL) {
		fp->message_header_fields = (char *)_ogg_calloc(this_message_size, sizeof(char));
    } else {
        int new_size = (fp->current_header_size + this_message_size) * sizeof(char);
		fp->message_header_fields = (char *)_ogg_realloc(fp->message_header_fields, new_size);
    }
...
ogg_packet ogg_from_fishead(fishead_packet *fp) {
	...
	op.packet = (unsigned char *)_ogg_calloc(FISHEAD_SIZE, sizeof(unsigned char));
...
ogg_packet ogg_from_fisbone(fisbone_packet *fp) {
    ...
    memset (&op, 0, sizeof (op));       
	op.packet = (unsigned char *)(_ogg_calloc(packet_size, sizeof(unsigned char)));
...
fisbone_packet fisbone_from_ogg(ogg_packet *op) {
	...
	fp.message_header_fields = (char *) _ogg_calloc(fp.current_header_size + 1, sizeof(char));
